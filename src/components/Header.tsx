...
import { Autocomplete, Box, Button, createFilterOptions, Dialog, DialogActions, DialogContent, DialogTitle, InputLabel, Link, List, ListItemButton, ListItemText, TextField, Typography } from "@mui/material";
...
export const Header: React.FC<{
    ...
    setOrCreateNewActiveConfig: (config: LootFilterConfigOption & { sourceConfig?: FilterConfig }) => void
    ...
    const handleCopyFilter = (sourceName: string) => {
        const sourceConfig = data.configs.find((group) => group.name === sourceName);
    ...
    const handleDelete = (option: LootFilterConfigOption) => {
        setValue(
            filterConfigs.find((c) => c.label !== option.label) ?? null
        );
        deleteConfig(option);
    ...
    return (
        <Box>
            ...
            >
                <Typography variant="h4" color="primary">
                    ...
                    >
                        <span style={{ color: colors.rsYellow }}>
                            ...
                            <Link
                                style={{ color: colors.rsYellow, textDecorationColor: colors.rsYellow }}
                                target="_blank"
                ...
                <Box sx={{ marginLeft: 'auto' }}>
                    <Button variant="outlined"
                        ...
                        onClick={() => { if (value != null) { setDeleteDialogOpen(true) } }}>
                        Delete
                        <DeleteOutline />
                    </Button>
                    <Dialog open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)}>
                        ...
                        <DialogContent>
                            <Button
                                variant="contained"
                                color="error"
                                onClick={() => { if (value != null) { handleDelete(value) } }}
                            >
                                Delete
                            </Button>
                        </DialogContent>
                ...
                <Autocomplete
                    ...
                    renderInput={(params) =>
                        <div>
                            <InputLabel id="config-select-label" sx={{ color: colors.rsOrange }}>Config Select</InputLabel>
                            <TextField
                                variant="standard"
            ...
            <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)}>
                <DialogTitle sx={{ color: colors.rsYellow }}>Create New Filter</DialogTitle>
                <DialogContent>
